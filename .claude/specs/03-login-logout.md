...
## Overview
Upgrade the stub `GET /login` route into a fully functional authentication flow and implement the `GET /logout` stub. Users can sign in with their email and password; on success a session is started and they are redirected to the dashboard (or profile until a dashboard exists). Logout clears the session and redirects to the landing page. The navbar in `base.html` is updated to show context-sensitive links — "Sign in / Get started" for guests, and "Sign out" for authenticated users.
...
- `POST /login` — validate credentials against DB, start session, redirect to `/profile` — public
- `GET /logout` — clear session, redirect to `/` — logged-in (safe to call as guest too; just redirects)

## Database changes
No new tables or columns.

A new DB helper must be added to `database/db.py`:
- `get_user_by_email(email)` — queries `users` by email and returns the row as a `sqlite3.Row` (or `None` if not found). Used by the login route to look up the user before verifying the password.
...
- **Modify**: `templates/base.html`
  - The `<div class="nav-links">` block currently always shows "Sign in" and "Get started"
  - Add a Jinja2 `{% if session.user_id %}` branch:
    - Logged-in: show a greeting ("Hi, {{ session.user_name }}") and a "Sign out" link pointing to `url_for('logout')`
    - Guest: keep existing "Sign in" and "Get started" links

## Files to change
- `app.py` — upgrade `login()` to handle `GET` and `POST`; import `get_user_by_email`; implement `logout()`
...
- Guard against already-logged-in users hitting `GET /login`: redirect them to `/profile` if `session.user_id` is already set
...
- [ ] Submitting a correct email but wrong password shows the same generic error message
- [ ] `GET /logout` clears the session and redirects to the landing page
- [ ] After logout, accessing `/login` shows the guest form (session is fully cleared)
- [ ] Navbar shows "Sign in / Get started" for guests and "Hi, \<name\> / Sign out" for logged-in users
- [ ] Flash messages in `login.html` render using the same pattern as `register.html`
